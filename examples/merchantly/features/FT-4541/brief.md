...
purpose: "Canonical brief для перехода фискализации с OrangeData на CloudKassir. Фиксирует problem space, scope и verify без смешения с solution space."
derived_from:
...
На стадии `Problem Ready` unresolved blocking decisions отсутствуют. Все принятые локальные решения по выбранному подходу вынесены в [`design.md`](design.md).
