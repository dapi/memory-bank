...
9. **Связь с issue.** При создании feature package агент обязан добавить комментарий в GitHub issue со ссылками на `feature.md` и (после появления) `implementation-plan.md`. Формат: permalink на файлы в ветке (`blob/master/` или `blob/<branch>/`). Это обеспечивает навигацию из issue к спецификации без поиска по репозиторию.
