...
конкретными путями на одной repository revision.
...
| Term | Meaning |
| --- | --- |
| `working context` | Информация и инструкции, доступные исполнителю в текущем запуске. Это не долговременный SSoT и может быть неполным. |
| `review` | Ограниченная проверка артефакта или изменения по явным критериям с зафиксированным результатом. Она сама по себе не разрешает менять проверяемый объект. |
| `audit` | Систематическая проверка набора артефактов на полноту, связность или соблюдение правил; обычно шире review одного изменения. |
| `simplify review` | Отдельный проход после функциональной проверки: поиск случайной сложности, дублирования и ненужных абстракций без изменения требуемого поведения. |
| `autonomy boundary` | Граница действий, которые агент может выполнить сам, под наблюдением или только после эскалации. |
| `human gate` | Остановка, когда требуется решение человека из-за существенной неизвестности, конфликта или риска. |
| `approval gate` | Явное согласование конкретного рискованного действия или действия с внешним эффектом. |
| `approval ref` | Ссылка на доказательство согласования: задачу, review, комментарий или ручную отметку принятого формата. |
| `supervision` | Режим, в котором агент готовит действие или план, но показывает его человеку в установленной точке контроля. |
| `escalation` | Передача вопроса человеку, потому что агент не вправе или не может безопасно принять решение. |
| `stop condition` | Условие немедленной остановки процесса. |
| `destructive action` | Удаляющее или труднообратимое действие, требующее особой осторожности и обычно согласования. |
| `blocker` | Неразрешённое условие, которое не позволяет безопасно продолжить работу или принять результат. |
| `finding` | Подкреплённое наблюдение, полученное во время review или audit. |
| `verdict` | Итоговая оценка проверки, например `pass`, `pass_with_notes` или `fail`. |
| `severity` | Серьёзность finding: `critical`, `important` или `minor`; способность заблокировать конкретный переход оценивается отдельно. |
| `Definition of Done` (`DoD`) | Общие условия, при которых задача или стадия действительно завершена. Это не заменяет acceptance criteria конкретного результата. |

## Planning And Verification Terms

| Term | Meaning |
| --- | --- |
| `acceptance criteria` | Условия, по которым принимается конкретный результат. Они не задают реализацию теста и не заменяют общую Definition of Done. |
| `implementation plan` | Производный технический план исполнения принятого результата и выбранного решения. |
| `execution space` | Техническое исполнение: контекст исследования, шаги, проверки и согласования. |
| `grounding` | Исследование текущего состояния системы до составления implementation plan. |
| `evidence contract` | Договорённость о форме, источнике и месте доказательств, нужных для проверки. |
| `checkpoint` | Промежуточная контрольная точка, которая не завершает всю поставку. |
