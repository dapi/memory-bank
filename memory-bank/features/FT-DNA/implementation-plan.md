...
| --- | --- | --- | --- |
| STEP-00 | Независимый reviewer: artifact review draft plan; после clean автор переводит plan в active, фиксируется новый immutable candidate и reviewer проверяет его заново. Этот шаг выдаёт Plan Ready verdict до любых payload edits | CP-01 / revision-bound Git notes с двумя verdicts; code-converge document mode, zero fix budget | brief/design active; PRE-01 к этому review не применяется |
