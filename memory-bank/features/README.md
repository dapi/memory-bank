...
В шаблонном репозитории реестр может быть пустым. Это нормально: реальные
`FT-XXX/` packages создаются в downstream-проектах или в этом репозитории
только на время delivery и не являются частью переносимого шаблона.
...
В шаблонной копии здесь нет instantiated packages.
