...
If the project has no reusable UI kit or separate frontend/admin UI, leave this guide minimal or remove it only together with the index links that point here. The canonical frontend engineering contract stays in [`../frontend.md`](../frontend.md); this guide records concrete local UI reference material.
