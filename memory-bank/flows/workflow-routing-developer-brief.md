...
Этот brief можно адаптировать для команды или вставлять в issue/PR instructions. Canonical routing rules остаются в [routing.md](routing.md); [workflow-decision-log.md](workflow-decision-log.md) объясняет причины changes to selector, а [workflow-metrics.md](workflow-metrics.md) помогает проверить, что profiles не ухудшили safety/evidence.
...
| New or materially changed capability, contract/risk trigger, design reasoning or acceptance traceability | Feature package | `memory-bank/features/FT-XXX/` following [feature.md](feature.md) |
| Multiple independent delivery units, roadmap, risk register or subissues | Epic package | `memory-bank/epics/EP-XXX/` following [epic.md](epic.md) |
