...
| `ui-reference/mockups/*.md` или другой linkable artifact | Создавай, когда interface change нельзя reliably review по prose и existing UI references; для trivial copy/state tweaks достаточно rationale в `brief.md` или `implementation-plan.md` | screen sketch, state examples, interaction notes | canonical acceptance inventory или final visual design system |
