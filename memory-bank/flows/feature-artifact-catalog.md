...
| Privacy / source boundary | Какие источники можно читать, хранить, цитировать и использовать как evidence? | Logs, transcripts, diagnostics, external metadata или источники с privacy/access constraints | `privacy-source-boundary.md` | Source inventory и feature-local use boundaries; requirements и global policy остаются у canonical owners | [Privacy / Source Boundary](templates/feature/support/privacy-source-boundary.md) |
| UI flow / mockups | Что видит пользователь и какие interface states проходит? | Меняется UI, navigation, editor/preview или interaction model | `ui-reference/README.md`, `ui-reference/mockups/*` | Interface reference; requirements и selected solution остаются у canonical owners | [UI Reference](templates/feature/support/ui-reference.md) |
...
| C4 view | Какие system, container, component или critical code boundaries затронуты? | Срабатывает C4 trigger из feature flow | Embedded Mermaid/table; при росте `diagrams/<name>-c4.md` | Reference projection `C4-*`, `SOL-*`, `SD-*`, `CTR-*` или accepted ADR | pattern in Design |
...
| Data-flow diagram | Откуда приходят данные, как преобразуются и куда уходят? | Несколько sources/sinks, transformations или data owners | Embedded diagram; при росте `diagrams/<name>-data-flow.md` | Reference projection canonical contracts and ownership | pattern only |
...
| API / interface contract | Как взаимодействуют producer и consumer? | Detailed API/event/schema/file/provider boundary | Inline `CTR-*`; при росте `contracts/<name>.md` | Delegated owner explicitly listed `CTR-*`; selected solution остается в `design.md` | [API Contract](templates/feature/api-contract.md) |
| Event catalog / schema | Какие events публикуются/потребляются и как versioned? | Event-driven interaction или очередь | API contract variant or `contracts/<event>.md` | Delegated event `CTR-*` | API Contract variant |
...
| Error taxonomy | Какие errors/states существуют и как consumer их интерпретирует? | API/integration или много failure outcomes | Table in contract or `design.md` | `CTR-*` wire semantics and `FM-*` solution behavior | API Contract pattern |
...
| Idempotency / concurrency model | Как обрабатываются duplicates, races, locks и ordering? | Callbacks, jobs, financial operations или parallel writers | Contract/design tables and sequence branches | `CTR-*`, `INV-*`, `FM-*`, `SD-*` | API/Sequence patterns |
...
| Compatibility matrix | Какие producer/consumer/schema versions совместимы? | Rolling deploy или independently released components | Table in contract/migration design | Delegated compatibility contract | API/Migration pattern |
