...
- `template/memory-bank/` — upstream payload, который должен оставаться generic; CLI устанавливает его downstream как `memory-bank/`.
...
Новые generic-правила размещайте в `template/memory-bank/`. Конкретную project-specific specialization не возвращайте обратно в шаблон.
...
- `rg --files template/memory-bank` для проверки структуры и имён файлов;
