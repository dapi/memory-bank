...
| `task-capsule.md` | Как продолжить активную задачу между sessions, machines или roles без полного transcript? | Работа пересекает session/machine/role boundary или требует persisted handoff | `features/FT-XXX/task-capsule.md` или `epics/EP-XXX/task-capsule.md` | Structured continuation state; canonical requirements, solution and lifecycle remain with their owners | [Task Capsule](templates/process/task-capsule.md) |
