...
- [PROC-XXX: Process Documentation Index](process/README.md) — шаблон индекса процесс-документов. Отвечает на вопрос: как собрать routing-layer для reusable process cards и lifecycle protocol.
...
[Execution Handoff Contract](../execution-handoff.md) — единый контракт
проверяемой read-only проекции исполнения с первичными источниками.
