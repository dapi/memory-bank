...
Контракт применяется к Feature, Small Change, Bug Fix и Refactoring Flow. Он не
относится к artifact review до lifecycle gate — в частности к Plan Ready, у
которого свой бюджет и свои объекты проверки; их разделение описано в
[`Verification Context Separation`](#verification-context-separation). Incident
Flow закрывает containment по своим правилам: profile ему не назначается, а
permanent remediation получает и profile, и этот контракт после отдельного
...
До выполнения этих условий работа не переходит в конечное состояние выбранного flow — `Done`, `Resolved` или эквивалентное.

### Бюджет и граница цикла

Один цикл — это `несошедшаяся проверка → исправления → повторная проверка`.
Несошедшейся считается любая: verdict с блокирующими замечаниями **или** красный
обязательный CI по причине в коде. Цикл заканчивается следующей проверкой,
сколько бы коммитов ни содержали исправления. Проверка без изменения редакции
циклом не является.

Считать только замечания reviewer недостаточно: петля «чистый verdict → красный
CI → правка → снова чистый verdict → снова красный CI» не содержит ни одного
verdict-а с замечаниями и без этого правила не расходовала бы бюджет вовсе.
...
Правая колонка называет владельца фактов и действие; gates в скобках относятся
к Feature Flow. Flow без такого этапа возвращает работу к соответствующему месту
своего lifecycle, а если этапа нет вовсе — повторяет
[`Task Routing`](routing.md). Так, Small Change Flow по своим entry predicates
не имеет ни design, ни execution plan: причина, требующая их, там же и является
триггером rerouting.
...
След фиксируется там же, где живёт validation profile decision этого flow —
в canonical owner из таблицы
[`Canonical Decision Owner By Flow`](validation-profiles.md#canonical-decision-owner-by-flow).
Один носитель, а не любой подходящий: routing record и profile decision у Feature
Flow лежат в разных документах, и без этого правила у записи было бы два
одинаково законных места.

Минимальная запись:

```text
Review convergence: converged | not-converged
Cycle: <номер> / <бюджет>
Reviewed revision: <commit SHA>
Verdict: <clean | findings>
Cause class: <строка таблицы выше; только при not-converged>
Decision: continue | stop | return to <владелец фактов>
```
