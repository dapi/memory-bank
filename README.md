...
Во всех сценариях выбирайте подходящий workflow в [`memory-bank/flows/workflows.md`](memory-bank/flows/workflows.md): не каждой небольшой задаче нужен полный feature package. Поддерживайте индексы и относительные ссылки и запускайте [локальные проверки](#проверка-ссылок-и-индексации) перед коммитом.
